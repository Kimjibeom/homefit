...
당신의 역할은 사용자의 대략적인 가용 예산(보유 자금 + 예상 대출금)을 계산하여 max_price를 산정하고,
search_properties 도구를 호출하는 것입니다.
도구가 반환한 점수 기반 TOP 5 매물 목록을 그대로 Finance Expert에게 전달하세요.
단 1회만 검색합니다.

행동 규칙:
1. 반드시 search_properties 도구를 1회 호출하세요.
2. max_price는 아래에 제시된 '추천 max_price'를 사용하세요.
3. 도구가 반환한 TOP 5 결과를 그대로 출력하세요. 추가 검색이나 재호출은 하지 않습니다."""
...
def run_property_matcher_agent(state: GraphState) -> dict:
    """사용자 프로필 기반으로 Tool Calling 1회로 예산 내 TOP 5 매물을 검색합니다.
    ...
    human_content = (
        ...
        f"위 조건에 맞는 매물을 검색하고 TOP 5 결과를 반환해 주세요."
    )
    ...
    try:
        ...
        if not candidates:
            candidates, _ = _fallback_search_list(profile, suggested_max)

        selected = candidates[0] if candidates else {"error": "적합한 매물을 찾지 못했습니다."}
...
Property Matcher가 전달한 TOP 5 매물들에 대해 정밀 재무 검증을 수행합니다.
각 매물별로 취득세와 정확한 대출 한도(LTV, DSR)를 계산하여,
최종적으로 구매 가능한 매물들의 상세 리포트를 표 형태로 출력하세요.
표에는 [순위, 매물명, 입지점수, 실시간 매매가, 취득세, 필요 대출금]이 포함되어야 합니다.
...
[분석 절차 — TOP 5 매물 각각에 대해 아래 순서를 반복하세요]
...
3단계. 대출 필요 여부 분기:
   - can_buy_without_loan=true → 대출 불필요
   - can_buy_without_loan=false → 4단계로 진행
4단계. 최대 대출 한도 산출: calculate_loan_limit 도구를 호출합니다.
   ★ 반드시 user_budget, acquisition_tax, area, is_first_home 인자를 전달하세요.
5단계. 최종 검증: needed_loan_만원 ≤ max_loan_limit_만원 → 구매 가능(PASS), 초과 → 구매 불가(FAIL)

[최종 리포트 — 마크다운으로 아래 항목을 반드시 포함]
1. 사용자 재무 현황 요약
2. TOP 5 매물 종합 분석 결과표
   | 순위 | 매물명 | 입지점수 | 실시간 매매가 | 취득세 | 필요 대출금 | 구매 가능 여부 |
3. 구매 가능 매물에 대한 상세 설명
4. 종합 의견 및 추천

리포트 마지막에 반드시 아래 태그로 판정 결과를 표기하세요 (구매 가능 매물이 1개라도 있으면 PASS):
  [VERDICT]PASS[/VERDICT]  또는  [VERDICT]FAIL[/VERDICT]"""


def run_finance_expert_agent(state: GraphState) -> dict:
    """RAG 기반 정책 검색 + Tool Calling으로 TOP 5 매물 각각의 구매 가능성을 일괄 검증합니다.
    ...
    for i, c in enumerate(top5, 1):
        top5_lines.append(
            f"  {i}. {c.get('name', 'N/A')} (ID: {c.get('id', 'N/A')}) | "
            f"입지점수 {c.get('score', 'N/A')} | "
            f"실시간 매매가 {c.get('price', 0):,}만원 | "
    ...
    human_content = (
        f"## 사용자 재무 현황\n"
        f"- 보유 자금: {user_budget:,}만원\n"
        f"- 연소득: {profile.get('annual_income', 0):,}만원\n"
        f"- 생애 첫 주택: {'예' if is_first_home else '아니오'}\n"
        f"- 기존 연간 대출 상환액: {profile.get('existing_debt_payment', 0):,}만원\n\n"
        f"## Property Matcher가 전달한 TOP 5 매물 (입지점수 내림차순)\n"
        f"{top5_text}\n\n"
        f"위 TOP 5 매물 각각에 대해 분석 절차(1~5단계)를 수행하고,\n"
        f"최종 리포트를 마크다운 표 형태로 출력하세요.\n"
        f"표에는 [순위, 매물명, 입지점수, 실시간 매매가, 취득세, 필요 대출금, 구매 가능 여부]를 포함하세요."
    )
    ...
    try:
        ...
        feedback = "" if is_valid else "TOP 5 매물 중 구매 가능한 매물이 없습니다."
