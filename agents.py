...
def _pick_best_property(candidates: list, profile: dict) -> dict:
    """후보 리스트에서 가장 비싸면서 가구원 수에 적합한 매물을 선택합니다."""
    if not candidates:
        return {"error": "적합한 매물을 찾지 못했습니다."}

    household = profile.get("household_size", 1)
    suitable = [p for p in candidates if p.get("rooms", 0) >= household]
    if suitable:
        return suitable[0]
    return candidates[0]
...
2단계. 총 필요 자금 산출: 매물 가격 + 취득세
3단계. 필요 대출금 산출: 총 필요 자금 - 사용자 보유 자금
4단계. 대출 필요 여부 분기:
   - 필요 대출금 ≤ 0원 → 대출 불필요 판정 (월 상환액 0원, 규제 검토 생략)
   - 필요 대출금 > 0원 → 5단계로 진행
5단계. 최대 대출 한도 산출: calculate_loan_limit 도구를 호출하여
   LTV 및 스트레스 DSR 3단계 규제를 적용한 은행 최대 대출 한도를 도출합니다.
   ★ 도구 호출 시 반드시 user_budget, acquisition_tax, area, is_first_home 인자를 전달하세요.
6단계. 최종 검증: 필요 대출금 ≤ 최대 대출 한도 → 구매 가능(PASS), 초과 → 구매 불가(FAIL)
7단계. 월 상환액 계산: 최대 한도가 아닌 실제 빌려야 하는 '필요 대출금'을 기준으로 산정
...
6. 최종 구매 가능 여부 판정
7. 현재 자금으로 구매 가능한 {{preferred_area}} 지역 추천 아파트 Top 5
   → 이 섹션에 상세 분석한 1위 매물을 포함하여 가격 상위 5개 매물을 리스트(매물명, 가격, 면적)로 출력
