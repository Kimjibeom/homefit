...
7. 보유 자금 + 대출을 활용하여 구매 가능한 {{preferred_area}} 지역 추천 아파트 Top 5
   ...
   → 각 매물에 대해 순위·매물명·매매가·면적·지역·예상 필요 대출금을 표시
...
def run_finance_expert_agent(state: GraphState) -> dict:
    ...
    if top5:
        ...
        for i, c in enumerate(top5, 1):
            ...
            needed = max(c_price - user_budget, 0)
            top5_lines.append(
                f"  {i}. {c.get('name', 'N/A')} | "
                f"매매가 {c_price:,}만원 | "
                f"{c.get('size_m2', 'N/A')}m² | "
                f"{c.get('area', 'N/A')} | "
                f"예상 필요 대출금 약 {needed:,}만원"
            )
        top5_text = "\n".join(top5_lines)
    ...
    human_content = (
        ...
        f"리포트 7번 항목에는 위 Top 5를 마크다운 표(순위/매물명/매매가/면적/지역/예상 필요 대출금)로 출력하세요.\n"
